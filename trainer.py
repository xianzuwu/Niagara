...
class Trainer(nn.Module):
    def __init__(self, cfg, train_dataset_size):
        super().__init__()

        self.cfg = cfg
        self.step = 0
        self.model = GaussianPredictor(cfg)
        self.num_total_steps = (train_dataset_size // cfg.data_loader.get("batch_size", 1)) * cfg.optimiser.num_epochs
    ...
    def log_time(self, batch_idx, duration, loss):
        """
        Print a logging statement to the terminal, including additional metrics.
        """
        batch_size = self.cfg.data_loader.get("batch_size", 1)
        samples_per_sec = batch_size / duration
        time_sofar = time.time() - self.start_time
        training_time_left = (self.num_total_steps / self.step - 1.0) * time_sofar if self.step > 0 else 0
        print_string = ("epoch {:>3} | batch {:>6} | examples/s: {:5.1f} | " +
                        "loss: {:.5f} | time elapsed: {} | time left: {} | step: {:>6}")
        if batch_idx % 500 == 0:
            logging.info(print_string.format(self.epoch, batch_idx, samples_per_sec, 
    ...
    def log_scalars(self, mode, outputs, losses, lr):
        ...
        self.logger.log({f"{mode}/epoch": self.epoch}, self.step)
    ...
    def log(self, mode, inputs, outputs):
        ...
        for j in range(min(4, cfg.data_loader.batch_size)):
            for s in scales:
