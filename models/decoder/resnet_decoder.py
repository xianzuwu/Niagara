...
class ResnetNormalDecoder(nn.Module):
    """Pytorch module for a ResNet-based normal map decoder."""
    def __init__(self, cfg, num_ch_enc, use_skips=True):
        super(ResnetNormalDecoder, self).__init__()
        ...
        # Normal map has 3 channels: X, Y, Z components
        ...
        for s in self.scales:
            out = Conv3x3(self.num_ch_dec[s], self.num_output_channels)
            self.convs[("outconv", s)] = out
            nn.init.xavier_uniform_(out.conv.weight, cfg.model.normal_scale)
            nn.init.constant_(out.conv.bias, cfg.model.normal_bias)
    ...
    def forward(self, input_features):
        ...
        for i in range(4, -1, -1):
            ...
            x = self.convs[("upconv", i, 1)](x)
            if i in self.scales:
                normal_output = self.convs[("outconv", i)](x)
                # Normalize the normal map output
                #
                normal_output = torch.tanh(normal_output)
                normal_output = F.normalize(normal_output, p=2, dim=1)
                outputs[("normal", i)] = rearrange(normal_output, 'b n ... -> (b n) ...')
