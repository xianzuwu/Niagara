...
class Re10KDataset(data.Dataset):
    def __init__(self,
        ...
        if self.cfg.dataset.preload_normals:
            assert cfg.dataset.normal_path is not None
            self.normal_path = Path(self.cfg.dataset.normal_path)
        self.split = split
    ...
    def _load_sparse_pcl(self, seq_key, frame_idx=None):
        ...
        try:
            ...
            # print(f"Error when loading file {fn}: {e}")
            return None
        except Exception as e:
            # print(f"Unexpected error when loading file {fn}: {e}")
            return None
    ...
    def _load_normal(self, key, id):
        ...
        if os.path.exists(path / normal_file):
            ...
            normal_image = Image.open(path / normal_file).convert("RGB")  # Ensure it's 3-channel RGB
            
            # Convert the image to a NumPy array
            normal = np.array(normal_image).astype(np.float32) / 255.0  # Assuming normal map is in [0, 1] range
            
            # If the normal map was saved in [0, 1], rescale it to [-1, 1] for proper normal vectors
            normal = (normal - 0.5) * 2.0
            
        else:
            # print("Normal file {} does not exist", path / normal_file)
            normal = None
    ...
    def get_frame_data(self, seq_key, frame_idx, pose_data, color_aug_fn):
        ...
        if self.depth_path is not None:
            # print("depth path is not None")
            depth = self._load_depth(seq_key, frame_idx) 
            # print(f"Loaded depth at frame {frame_idx}: {depth}")
        ...
        return inputs_K_scale_target, inputs_K_scale_source, inputs_inv_K_source, inputs_color, inputs_color_aug, inputs_T_c2w, img.size, depth, normal
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if self.cfg.dataset.scale_pose_by_depth:
                    ...
                    if sparse_pcl is None:
                        raise ValueError("Sparse PCL is None")
                ...
                for frame_name, frame_idx in zip(frame_names, src_and_tgt_frame_idxs):
                    inputs_K_tgt, inputs_K_src, inputs_inv_K_src, inputs_color, inputs_color_aug, inputs_T_c2w, orig_size, inputs_depth, inputs_normal = self.get_frame_data(
                        seq_key=seq_key, frame_idx=frame_idx, pose_data=pose_data, color_aug_fn=color_aug)
                    ...
                    if inputs_normal is not None:
                        inputs[("normal", frame_name, 0)] = inputs_normal
                ...
                print(f"ValueError processing index {index}: {e}")
                index = (index + 1) % len(self._seq_key_src_idx_pairs)
            except Exception as e:
                print(f"Unexpected error processing index {index}: {e}")
                index = (index + 1) % len(self._seq_key_src_idx_pairs)
        
        raise RuntimeError("Exceeded maximum attempts to load a valid data sample")