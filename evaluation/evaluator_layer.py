...
from evaluation.lpips_layer import LPIPSLayer
...
class Evaluator(nn.Module):
    def __init__(self, crop_border=True):
        ...
        self.psnr = lambda pred, gt: -10 * torch.log10(torch.mean((pred - gt) ** 2, dim=[1, 2, 3])).mean()

        # initial lpips layer
        self.lpips_layer = LPIPSLayer(net_type="vgg", normalize=True)
        ...
        self.metrics = {
            ...
            "lpips_layer": lambda pred, gt: self.lpips_layer(pred, gt)
        }
    ...
    def forward(self, img_pred, img_gt):
        ...
        results = {
            name: func(img_pred, img_gt) for name, func in self.metrics.items()
        }
