...
def evaluate(model, cfg, evaluator, dataloader, device=None, save_vis=False):
    ...
    for k in tqdm([i for i in range(len(dataloader.dataset) // cfg.data_loader.batch_size)]):
        ...
        if save_vis:
            out_dir = Path("/home/wuxianzu/Projects/flash3d/man/flash3d/visual_results/images550s")
        ...
        try:
            ...
            if cfg.dataset.name=="re10k":
                if cfg.dataset.test_split in ["pixelsplat_ctx1",
                                              ...
                                              "latentsplat_ctx1",
                                              "latentsplat_ctx2"]:
                    logging.warning("Failed to read example {}".format(k))
                    continue
            raise e
    ...
    for metric in metric_names:
        ...
        mean_val = np.mean(np.array(vals))
        logging.info(f"{metric}: {mean_val}")
...
def main(cfg: DictConfig):
    ...
    os.chdir(output_dir)
    logging.info("Working dir: %s", output_dir)
    ...
    if cfg.dataset.name == 'kitti':
        ckpt_dir = Path(cfg.run.weights_path)
        if ckpt_dir.exists():
            # resume training
    ...
    logging.info(json.dumps(score_dict_by_name, indent=4))
    if cfg.dataset.name=="re10k":
        print('kunkun')
        print(cfg.dataset.test_split)
